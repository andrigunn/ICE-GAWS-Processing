...
AWS = readtable(".\Input\Secondary data\AWS.csv", opts);
clear opts

ind_station = find(strcmp(AWS.Stationname,station));

% loading RCM data at the station
filename = sprintf('./Input/Secondary data/%s_3h_AWS_sites.nc',RCM);
if ~exist(filename,'file')
   filename = sprintf('./Input/Secondary data/%s_hourly.nc',RCM);
end
...
%     if size(tmp) == size(data_RCM.time)
%         tmp(tmp<-665) = NaN;
...
data_RCM = ResampleTable(data_RCM);
...
%% Renaming variables
    ...
    if contains(RCM,'CanESM')
        data_RCM.Snowfallmweq = data_RCM.Snowfallmweq/1000;
        data_RCM.Rainfallmweq = data_RCM.Rainfallmweq/1000;
    else
        data_RCM.Snowfallmweq = data_RCM.Snowfallmweq/1000/3;
        data_RCM.Rainfallmweq = data_RCM.Rainfallmweq/1000/3;
    end
    if ismember('rf',data_RCM.Properties.VariableNames)
        figure
        plot(data_RCM.time,cumsum(data_RCM.rf))
