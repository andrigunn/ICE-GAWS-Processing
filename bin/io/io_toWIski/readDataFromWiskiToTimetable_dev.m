function GAWS_L2_WData = readDataFromWiskiToTimetable_dev(stationName,getDataFrom,getDataTo,savedata,saveStationName)
...
% Breiðá er lesin eins fyrir Breiðarmerkujökul
switch stationName
    case 'Breiða'
        station_ind = matches({GawsStationsInWiski.station_name},stationName);
    case 'Dyngjujökull Dyn'  
        station_ind = matches({GawsStationsInWiski.station_name},stationName);
...
for i = 1:length(GetParameters)
    ...
    disp(['Getting data for ', char(Ptype), ' and ',...
        char(parName),' at ', GetParameters(i).station_name])
    ...
    clear tsData time value
    tsData = webread(getTimeseriesValuesURL);

    % If not ts data exist
...
if isempty(fieldnames(outTables)) == 1
    disp 'No data in table, not saving'
    GAWS_L2_WData = [];
else
